...
Les 3 grands principes de l’architecture hexagonale sont : 

* *Séparer explicitement Application, Domain, et Infrastructure*

On constate cette séparation dans l’arborescence du projet : 
====
    /application
        /port
            /in
...
* *Les dépendances vont vers le Domain*

La classe TodoItem appartenant au Domain est utilisée par les autres modules, mais elle ne dépend d’aucun d’entre eux.

* *Séparation des entrées et des sorties :* 

On retrouve deux classes exposant l’entité du domaine : 

    - in/GetTodoItems.java 
    - out/LoadTodoItems.java

Dans une application réelle, la classe LoadTodoItems, qui pourra être utilisée par une API n’exposera pas forcément tous les attributs de l’entité métier TodoItem. 

*Le out dépend du in mais jamais l’inverse*
...
Le principe d’inversion de dépendance est implémenté ici avec le pattern d’injection de dépendance (Dependency injection). 

.TodoItemPersistenceAdapter()
[source, java]
----
@Inject
...
L’anti-pattern domaine anémique consiste à créer des entités du domaine ne comportant peu ou pas de de règles de comportement métier. Un exemple type de domaine anémique serait une classe comportant uniquement des propriétés, getters et des setters, sans aucune règle de comportement ou métier. 

*_“The catch comes when you look at the behavior, and you realize that there is hardly any behavior on these objects, making them little more than bags of getters and setters.”_* Martin Fowler

L’anti-pattern domaine anémique se reconnaît facilement, en effet les classes domaines ne comprennent pas de règle métier, mais seulement les getters et setters. 

Etant donné que l’architecture hégagoanale vise à concentrer la gestion des comportements métier dans le domaine, on peut dire qu’elle permet d’éviter l’anti-pattern domaine anémique. 
